...
const background_model_url =
  "https://cdn.jsdelivr.net/gh/Eikanya/Live2d-model/%E5%B0%91%E5%A5%B3%E5%89%8D%E7%BA%BF%20girls%20Frontline/live2dold/bg/cg7/model.json";
/**
...
const openAIKey =
  helpers.getURLParam("openai") ||
  "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";
const usawServerURL = helpers.getURLParam("usaws");
const AIPossible = openAIKey != null || usawServerURL != null;
if (!AIPossible) {
  addError(
    "ERROR: No OpenAI API Key and no USAW server available. Either is required for her to think."
  );
}
if (openAIKey && usawServerURL) {
  addWarning(
    "WARNING: OpenAI API key and USAW server given. Defaulting to USAW"
  );
}
...
const AZURE_POSSIBLE =
  subscriptionKey != null && serviceRegion != null && !!window.SpeechSDK;
...
if (helpers.isMobile() && engineOfChoice == "native") {
  addWarning(
    "WARNING: To my knowledge, native TTS and speech recognition does not work on mobile. Try to use Azure."
  );
}
...
if (!SPEECH_POSSIBLE) {
  ...
  addWarning(
    "WARNING: Azure speech is misconfigured. Trying to fall back on native. Fix it by passing a GET parameter called 'speech_key' for the API key and 'speech_region' for the azure region"
  );
  voiceEngine = "native";
} else if (voiceEngine == "native" && !NATIVE_SPEECH_POSSIBLE) {
  addWarning(
    "WARNING: Native TTS is not available. You must use Azure. No voice - she is now a mute."
  );
  voiceEngine = null;
} else if (voiceEngine == "text") {
  addWarning(
    "WARNING: Text mode is enabled. She will not speak. You can change this by passing a GET parameter called 'tts-engine' or 'engine' with the value 'native' or 'azure'."
  );
  voiceEngine = null;
}

const NATIVE_SPEECH_RECOGNITION_POSSIBLE = "webkitSpeechRecognition" in window;
const SPEECH_RECOGNITION_POSSIBLE =
  NATIVE_SPEECH_RECOGNITION_POSSIBLE || AZURE_POSSIBLE;
var speechRecognitionEngine =
  helpers.getURLParam("sr-engine") || engineOfChoice;
if (!SPEECH_RECOGNITION_POSSIBLE) {
  addWarning(
    "WARNING: Speech recognition not available. She is switching to text input mode."
  );
  ...
  speechRecognitionEngine == "native" &&
  !NATIVE_SPEECH_RECOGNITION_POSSIBLE
) {
  addWarning(
    "WARNING: Native speech recognition is not available. You must use Azure. No hearing - falling back on text input."
  );
  speechRecognitionEngine = "text";
}
...
if (voiceEngine != "azure" && sttAzureLang != "en-US") {
  addWarning(
    "WARNING: Using different languages for speech-to-text is only available with the Azure engine. Defaulting to english."
  );
}
...
if (voiceEngine == "azure") {
  ttsFactory = TextToSpeechSynthesizerFactory.Azure(
    model,
    "en-US",
    voice,
    subscriptionKey,
...
if (speechRecognitionEngine == "native") {
  ...
  sttFactory = SpeechToTextRecognizerFactory.Azure(
    sttAzureLang,
    subscriptionKey,
    serviceRegion
  );
  ...
  console.error(
    "Illegal speechRecognitionEngine state!",
...
(async function main() {
  ...
  function resizeBackground() {
    let scale = Math.max(
      window.innerHeight * 0.00048,
      window.innerWidth * 0.00027
    );
  ...
  model.internalModel.motionManager.groups.idle =
    modelData["idleMotionGroupName"] ?? "Idle";
